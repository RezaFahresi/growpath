...
import { 
  Zap, ChevronRight, Trophy, AlertTriangle, Target, Code2, Megaphone, 
  TrendingUp, Palette, Search, BookOpen, Monitor, Briefcase, PenTool, Layout, Globe, Smartphone, Server 
} from 'lucide-react';
import { useNavigate, useLocation } from 'react-router-dom';
import { useAppContext } from '../context/AppContext';
import API from '../api/axios';

// Pemetaan Ikon agar string dari database bisa dirender sebagai komponen Lucide
const IconMap = {
...
export default function Roadmap() {
  ...
  const [availableRoadmaps, setAvailableRoadmaps] = useState([]); // State untuk Roadmap dari Database
  ...
  // Menarik data Roadmap LANGSUNG dari Database (Admin)
  useEffect(() => {
    const fetchRoadmapsData = async () => {
      try {
        setLoading(true);
        // Tarik data talent mapping user
        const talentRes = await API.get('/talent-mapping/me').catch(() => ({ data: null }));
        if (talentRes.data) setUserTalentData(talentRes.data);

        // Tarik data Roadmap yang dibuat oleh Admin
        const roadmapRes = await API.get('/roadmaps');
        setAvailableRoadmaps(roadmapRes.data || []);
      } catch (error) {
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-[2rem] p-8 flex flex-col md:flex-row items-center gap-6 shadow-sm">
          ...
          <div>
            ...
            <p className="text-amber-800/80 text-sm leading-relaxed">
              ...
              <button onClick={() => navigate('/dashboard/assessments')} className="font-bold text-indigo-600 underline hover:text-indigo-800">
      ...
      <div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-slate-500 font-bold animate-pulse mt-4">Menarik Data Kurikulum...</p>
          </div>
          ...
          <div className="col-span-full text-center py-16 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200 mt-6">
            ...
            <h3 className="text-slate-700 font-bold text-lg mb-1">
              {searchQuery ? `Roadmap "${searchQuery}" tidak ditemukan` : 'Belum ada Roadmap tersedia'}
            </h3>
            <p className="text-slate-500 text-sm">
              {searchQuery ? 'Coba gunakan kata kunci lain.' : 'Admin belum menambahkan roadmap ke dalam sistem.'}
            </p>
            {searchQuery && (
              <button 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
            {filteredRoadmaps.map((roadmap) => {
              const totalModules = (roadmap.items || []).length || 1; // Mencegah dibagi 0
              ...
              const isCompleted = completedItems === totalModules && totalModules > 0;
              
              // Menerjemahkan nama Ikon
              const IconComponent = IconMap[roadmap.icon_name] || BookOpen;
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-6">
                    <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${roadmap.theme || 'from-slate-800 to-slate-900'} border ${roadmap.border_color || 'border-slate-500/30'} flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300`}>
